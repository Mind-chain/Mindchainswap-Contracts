...
/**
 * @title MindchainSwap  Farms Auctions.
 * @notice Auctions for new Farms, including multiplier.
 */
contract MindFarmAuction is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Constructor
     * @param _mindToken: address of the $MIND token
     ...
     * @param _amount: amount of the bid in $MIND token
    ...
    function bid(uint256 _amount) external nonReentrant {
        ...
        mindToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function claimAuction(uint256 _auctionId) external nonReentrant {
        ...
        mindToken.safeTransfer(address(msg.sender), claimableAmount);
    ...
    /**
     * @notice Close an auction round, and store $MIND leaderboard threshold
     * @param _bidLimit: minimal $MIND committed to be a winner
     * @dev Callable by operator
     */
    ...
    /**
     * @notice Claim $MIND for all bids by bidders in leaderboard
     * @param _auctionId: auction id
     * @param _bidders: addresses of bidders
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(mindToken), "Recover: Cannot be MIND token");
